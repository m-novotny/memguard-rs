...
//!
//! // Create a 32-byte locked, zeroized-on-drop region
//! let mut region = GuardedRegion::<32>::new().unwrap();
//!
...
use crate::zeroize::{Zeroize, zeroize_slice};
...
impl<const N: usize> GuardedRegion<N> {
    ...
    /// # Errors
    ///
    /// Returns [`Error::LockFailed`] if the OS refuses to lock the
