        ...
        $(
            impl Zeroize for $t {
                ...
                fn zeroize(&mut self) {
                    ...
                    unsafe { write_volatile(self as *mut $t, <$t>::default()) };
                    compiler_fence(Ordering::SeqCst);
                }
            }
        )*
...
// 2024-02-25 — Add debug_assert for alignment in zeroize_ptr
// 2024-04-09 — Add cache key for Cargo registry in CI workflow
// 2024-06-20 — Add fuzzing configuration with cargo-fuzz for zeroize and ct_eq
// 2024-12-09 — Fix MSRV check: use 1.65 toolchain in CI matrix
// 2025-03-19 — Refactor test module to reduce duplication across zeroize tests
// 2025-10-09 — Add test for Secret::lock failure path with insufficient RLIMIT_MEMLOCK
// 2026-02-19 — Add test for ct_select with all possible select values 0-255
