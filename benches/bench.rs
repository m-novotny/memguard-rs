...
fn bench_zeroize_slice(c: &mut Criterion) {
    ...
    c.bench_function("zeroize_slice/4096", |b| {
        b.iter(|| {
    ...
    c.bench_function("zeroize_slice/32", |b| {
        b.iter(|| {
...
fn bench_ct_eq(c: &mut Criterion) {
    let a = vec![0x42u8; 32];
    let b = vec![0x42u8; 32];
    c.bench_function("ct_eq/32", |b| {
        b.iter(|| {
            black_box(ct_eq(black_box(&a), black_box(&b)))
        })
    });

    let a_large = vec![0x42u8; 4096];
    let b_large = vec![0x42u8; 4096];
    c.bench_function("ct_eq/4096", |b| {
        b.iter(|| {
            black_box(ct_eq(black_box(&a_large), black_box(&b_large)))
        })
    });
}

fn bench_secret_drop(c: &mut Criterion) {
    c.bench_function("secret_drop/32", |b| {
        b.iter(|| {
